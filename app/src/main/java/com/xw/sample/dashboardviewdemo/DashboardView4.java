...
public class DashboardView4 extends View {
    ...
    protected int mMax = 180; // 最大值
    protected int mSection = 9; // 值域（mMax-mMin）等分份数
    protected int mPortion = 2; // 一个mSection等分份数
...
//        canvas.drawArc(mRectFInnerArc, mStartAngle + 1, mSweepAngle - 2, false, mPaint);
        ...
        if (mVelocity >= 10000) {
            ...
            drawDigitalTube(canvas, mVelocity % 1000 / 100, -xOffset, isEngineSpeed);
            ...
            drawDigitalTube(canvas, -1, -xOffset, isEngineSpeed);
            ...
            drawDigitalTube(canvas, tmp < 10 ? -1 : -2, -xOffset, isEngineSpeed);
            drawDigitalTube(canvas, tmp < 10 ? -2 : (tmp / 10), 0, isEngineSpeed);
            ...
            drawDigitalTube(canvas, -1, -xOffset, isEngineSpeed);
            drawDigitalTube(canvas, -1, 0, isEngineSpeed);
            drawDigitalTube(canvas, mVelocity, xOffset, isEngineSpeed);
        }
    }

    private void drawDigitalTube(Canvas canvas, int num, int xOffset, boolean isEngineSpeed) {
        float x = mCenterX + (isEngineSpeed ? (xOffset + dp2px(10)) : xOffset);
        ...
        // 1
        mPaint.setAlpha(num == -2 || num == -1 || num == 1 || num == 4 ? 25 : 255);
        canvas.drawLine(x - lx, y, x + lx, y, mPaint);
        // 2
        mPaint.setAlpha(num == -2 || num == -1 || num == 1 || num == 2 || num == 3 || num == 7 ? 25 : 255);
        canvas.drawLine(x - lx - gap, y + gap, x - lx - gap, y + gap + ly, mPaint);
        // 3
        mPaint.setAlpha(num == -2 || num == -1 || num == 5 || num == 6 ? 25 : 255);
        canvas.drawLine(x + lx + gap, y + gap, x + lx + gap, y + gap + ly, mPaint);
        // 4
        mPaint.setAlpha(num == -1 || num == 0 || num == 1 || num == 7 ? 25 : 255);
        canvas.drawLine(x - lx, y + gap * 2 + ly, x + lx, y + gap * 2 + ly, mPaint);
        // 5
        mPaint.setAlpha(num == -2 || num == -1 || num == 1 || num == 3 || num == 4 || num == 5 || num == 7
                || num == 9 ? 25 : 255);
        canvas.drawLine(x - lx - gap, y + gap * 3 + ly,
                x - lx - gap, y + gap * 3 + ly * 2, mPaint);
        // 6
        mPaint.setAlpha(num == -2 || num == -1 || num == 2 ? 25 : 255);
        canvas.drawLine(x + lx + gap, y + gap * 3 + ly,
                x + lx + gap, y + gap * 3 + ly * 2, mPaint);
        // 7
        mPaint.setAlpha(num == -2 || num == -1 || num == 1 || num == 4 || num == 7 ? 25 : 255);
